...
describe("Note app", function () {
  ...
  describe("if logged in can be liked", function () {
    ...
    it("it can be deleted", function () {
      ...
      cy.get("html").should(
        "not.contain",
        '"A new blog submission created from cypress" by Cypress'
      );
    });
    ...
    describe.only("and multiple blogs exist", function () {
      ...
      it("blogs are ordered by number of likes", function () {
        cy.get(".blog>span.title-author").then((blogs) => {
          expect(blogs[0].textContent).to.equal(
            '"Blog submission 3" by Cypress'
          );
          expect(blogs[1].textContent).to.equal(
            '"Blog submission 2" by Cypress'
          );
          expect(blogs[2].textContent).to.equal(
            '"A new blog submission created from cypress" by Cypress'
          );
        });
      });
